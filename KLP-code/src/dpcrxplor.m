...
ub = [1, 1,1];
mu0 = [0, 0,0];
Sigma0 = diag([25, 25,25]);
nStep = 100;
fmin_1 = @(f, X)fmin_adares(f, X, 3, mu0, Sigma0, 1, 30, lb, ub, [], []);
...
[X_skl, ~, nEval_12] = seklpoints_greedylcvd3(3, fp, fmin_1, nPart,initial);
...
Z = reshape(p, nStep, nStep);
        ...
        subplot(2,2,1)
contour(t1, t2, Z, 'levelstep', 1.5, 'linewidth', 0.5),title('medpoints');
...
contour(t1, t2, Z, 'levelstep', 0.05, 'linewidth', 0.5),title('klpoints');
      ...
      subplot(2,2,3)  
        ...
        subplot(1,2,1)
         [c,h]=contourf(t1, t2, Z, 11, 'levelstep', 1.5, 'linewidth', 0.5),title('SKL points');
map = [0.52 0.8 0.92
0.8 0.7 0.8
0.98 0.94 0.90
1 0.75 0.79
